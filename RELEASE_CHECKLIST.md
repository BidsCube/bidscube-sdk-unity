...
Use this list before tagging and publishing a core SDK release. Current production pin referenced by apps: **`v1.2.9`**.

## Version and tag

- [ ] `package.json` field `"name"` is `com.bidscube.sdk`
- [ ] `package.json` field `"version"` matches the release (e.g. `1.2.9` for tag `v1.2.9`)
- [ ] Git tag name follows `v` + semver (e.g. `v1.2.9`) and matches `package.json` version
...
git commit -m "Release com.bidscube.sdk 1.2.9"
git tag v1.2.9
git push origin main && git push origin v1.2.9
```

Then create a **GitHub Release** from tag `v1.2.9` (title `v1.2.9`, notes from `CHANGELOG.md`).

## After release

- Bump companion packages (`com.bidscube.applovin.max`, `com.bidscube.levelplay`) so `package.json` → `dependencies` → `com.bidscube.sdk` matches **`1.2.9`** and re-tag those repos per their `RELEASE.md`.
